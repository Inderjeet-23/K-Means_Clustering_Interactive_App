...
# X, y = make_blobs(n_samples, centers=n_centers, random_state=42)
...
if n_points:
    ...
    # Voronoi 2d plot library to plot Voronoi diagram
    ...
    # if st.session_state.added_points is not None:
    #     for point in st.session_state.added_points:
    #         plt.scatter(point[:, 0], point[:, 1], marker='x', s=200, linewidths=3, color='red')  
    # ax.scatter(kmeans.cluster_centers_[:, 0], kmeans.cluster_centers_[:, 1], marker='*', s=300, c='r')
    # ax.set_xlabel('Feature 1')
    # ax.set_ylabel('Feature 2')
    # ax.set_title('KMeans Clustering')
    # ax.legend()
    ...
    # plot_voronoi(X, kmeans, st.session_state.added_points)
